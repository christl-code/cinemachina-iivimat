...
namespace iivimat
{
    ...
    public class ExitHand : Action
    {
        ...
        {
            ProximityHandCollider[] exitColliders = go.GetComponentsInChildren<ProximityHandCollider>();
            if (exitColliders != null)
            {
                foreach (ProximityHandCollider exitCollider in exitColliders)
                {
